...
<body>
  ...
  <div class="form-card">
    <h3><?= $editing_id ? 'Edit' : 'Add New' ?> Skills</h3>
    <form method="post" enctype="multipart/form-data">
      
        <label>Category (e.g., programming, databases, ml, frontend, backend, tools, soft, languages)</label>
        <input name="category" value="<?= htmlspecialchars($item['category'] ?? '') ?>" required>
        <label>Name</label><input name="name" value="<?= htmlspecialchars($item['name'] ?? '') ?>" required>
        <label>Experience</label><input name="experience" value="<?= htmlspecialchars($item['experience'] ?? '') ?>">
        <label>Percent (0-100)</label><input name="percent" type="number" min="0" max="100" value="<?= htmlspecialchars($item['percent'] ?? '') ?>">
  ...
  <table>
    <thead>
      <tr><th>Category</th><th>Name</th><th>Percent</th><th>Order</th><th></th></tr>
    </thead>
    <tbody>
