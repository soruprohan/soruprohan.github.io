...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($editing_id) {
      $stmt = $mysqli->prepare("UPDATE testimonials SET title=?, description=?, image_path=?, order_index=? WHERE id=?");
      $stmt->bind_param('sssii', $title, $description, $image_path, $order_index, $editing_id);
    } else {
      $stmt = $mysqli->prepare("INSERT INTO testimonials (title, description, image_path, order_index) VALUES (?, ?, ?, ?)");
      $stmt->bind_param('sssi', $title, $description, $image_path, $order_index);
    }
    $stmt->execute();
  ...
  <div class="container">
    ...
    <div class="form-card">
      ...
      <form method="post" enctype="multipart/form-data">
          ...
          <label>Image (upload)</label><input type="file" name="image_file" accept="image/*" <?= $editing_id ? '' : 'required' ?>>
    ...
    <table>
      <thead>
        <tr><th>Title</th><th>Description</th><th>Image</th><th>Order</th><th>Actions</th></tr>
      </thead>
      <tbody>
        <?php foreach ($rows as $row): ?>
          <tr>
            <td><?= htmlspecialchars($row['title']) ?></td>
            <td class="description-preview"><?= htmlspecialchars($row['description']) ?></td>
            <td><?= !empty($row['image_path']) ? 'Yes' : 'No' ?></td>
